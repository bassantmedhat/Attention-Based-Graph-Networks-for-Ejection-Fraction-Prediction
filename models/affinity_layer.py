...
class Affinity(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        # Get the batch size and the number of samples (n_samples)
        batch_size, n_samples, C = X.size()

        # Expand X and Y for pairwise computation
        X_k = X.unsqueeze(2).expand(batch_size, n_samples, n_samples, C)
        Y_k = Y.unsqueeze(1).expand(batch_size, n_samples, n_samples, C)

        # X_k = X.unsqueeze(1).expand(N1, N2, C)
        # Y_k = Y.unsqueeze(0).expand(N1, N2, C)
        M = torch.cat([X_k, Y_k], dim=-1)
        M = self.fc_M(M).squeeze(-1)
